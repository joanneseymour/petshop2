  ...
  init: function() {
    ...
    $.getJSON('pets.json', function(data) {
      ...
      for (i = 0; i < data.abi.length; i ++) {
        ...
        petTemplate.find('.pet-age').text(data.abi[i].age);
        petTemplate.find('.pet-location').text(data.abi[i].location);
        petTemplate.find('.btn-adopt').attr('data-id', data.abi[i].id);
